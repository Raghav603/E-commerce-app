      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100 }}>
        ...
        <View style={styles.detailsContainer}>
          <View style={styles.titleRow}>
            <Text style={[styles.productTitle, { color: c.text }]} numberOfLines={2}>
              <Text style={{ fontWeight: 'bold' }}>{detailedProduct.brand ? `${detailedProduct.brand.toUpperCase()} ` : ''}</Text>
              {detailedProduct.title || detailedProduct.name}
            </Text>
          ...
          <View style={styles.ratingRow}>
            <View style={[styles.ratingBadge, { backgroundColor: c.green }]}>
              <Text style={styles.ratingText}>{detailedProduct.rating || '4.0'} ★</Text>
            </View>
            <Text style={[styles.reviewCount, { color: c.subText }]}>
              ({detailedProduct.reviews?.length || Math.floor(Math.random() * 1000) + 100} reviews)
            </Text>
            {/* Show Stock Status if available */}
            {detailedProduct.stock !== undefined && (
               <Text style={[styles.stockStatus, { color: detailedProduct.stock > 0 ? c.green : c.badge }]}>
        ...
        {product.description && (
          <View style={[styles.extraSection, { borderTopColor: c.border }]}>
            ...
            <Text style={[styles.descriptionText, { color: c.subText }]}>{detailedProduct.description}</Text>
          </View>
        )}
        ...
        <View style={[styles.extraSection, { borderTopColor: c.border }]}>
          ...
          {renderSpecRow('Category', getSpecValue(detailedProduct.category))}
          {renderSpecRow('Brand', getSpecValue(detailedProduct.brand))}
          {renderSpecRow('SKU', getSpecValue(detailedProduct.sku))}
          {renderSpecRow('Weight', getSpecValue(detailedProduct.weight))}
          {renderSpecRow('Dimensions', getSpecValue(detailedProduct.dimensions))}
          {renderSpecRow('Warranty', getSpecValue(detailedProduct.warrantyInformation))}
          {renderSpecRow('Shipping', getSpecValue(detailedProduct.shippingInformation))}
          {renderSpecRow('Return Policy', getSpecValue(detailedProduct.returnPolicy))}
        </View>
        ...
        <View style={[styles.extraSection, { borderTopColor: c.border }]}>
          ...
          {(detailedProduct.reviews && detailedProduct.reviews.length > 0 ? detailedProduct.reviews : [
