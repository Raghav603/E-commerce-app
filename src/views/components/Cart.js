...
const Cart = ({ 
  ...
  const renderCartItem = ({ item }) => {
    const { originalPrice, finalPrice, discountPct } = item;
  ...
  return (
    <View style={[styles.container, { backgroundColor: c.background }]}>
      <FlatList
        ...
        scrollEnabled={false}      // 👈 Add this: disable FlatList scrolling so parent takes over
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              if (onCheckoutPress) {
                ...
                navigateTo('ReviewOrder');
              }
            }} 
          >
            <Text style={styles.checkoutBtnText}>{checkoutButtonText}</Text>
          </TouchableOpacity>
        </View>
      )}
    </View>
